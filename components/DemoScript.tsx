  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <div key={step} className="demo-step-enter mt-6">
      {step === 0 && <DemoPanel eyebrow="Step 1 / Create Account" title="Your fan identity starts here" description="Use a demo Google identity to create your FanQuest profile. No real Google data is requested.">
        <div className="mx-auto max-w-md rounded-lg border border-white/10 bg-black/30 p-5"><div className="flex items-center gap-4"><span className="grid h-12 w-12 place-items-center rounded-full bg-white text-xl font-black text-blue-600">G</span><div><p className="font-black text-white">Google demo account</p><p className="text-sm text-slate-400">aiko.demo@gmail.com</p></div></div><label className="mt-5 block"><span className="text-sm font-bold text-slate-200">Fan display name</span><input className="focus-ring mt-2 w-full rounded border border-white/15 bg-slate-950 px-4 py-3 text-white" value={displayName} onChange={(event) => setDisplayName(event.target.value)} maxLength={20} /></label></div>
